@extends('layouts.user_layout')
@section('content')

<div class="container">

    <h2>Courses</h2>
    <div class="col-sm-3">
        {!! Form::open(['action' => 'Admin\AdminCoursesController@searchcourses', 'method' => 'POST']) !!}
        <input type="text" class="form-control round-form" name="searchcourses" id="searchcourses" placeholder="Search....">
        {!! Form::close() !!}
    </div>
    <div class="col-lg-6 col-md-6 col-sm-12">
        <button id="myBtn" type="button" class="btn btn-round btn-success" data-toggle="modal" data-target="#myModal"
            style="margin-left: 130%; margin-bottom: 5%;">Create Course</button>
    </div>
    <table class="table">
        <thead>
            <tr>
                <th>@sortablelink('id','ID')</th>
                <th>@sortablelink('course_name','Course')</th>
                <th>@sortablelink('college_id','College')</th>
            </tr>
        </thead>
        <tbody>
            @if($courses)

            @foreach($courses as $course)
            <tr>
                <td>{{$course->id}}</td>
                <td>{{$course->course_name}}</td>
                <td>{{$course->college ? $course->college->college_name : 'Uncategorized'}}</a></td>
                <td>
                    <a href="{{route('courses.edit', $course->id)}}" class="btn btn-primary">Edit</a>
                </td>
                <td>
                    <button type="button" class="btn btn-danger" data-toggle="modal"
                        data-target="#promptModal" data-course="{{$course->id}}">Delete</button>
                </td>
            </tr>

            @endforeach
            @endif

        </tbody>
    </table>
    {{$courses->links()}}
</div>
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Create Course</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                @include('includes.form_error')
                {!! Form::open(['method'=>'POST', 'action'=> 'Admin\AdminCoursesController@store'])!!}
                @csrf
                <div class="form-group">
                    {!! Form::label('course_name', 'Course:') !!}
                    {!! Form::text('course_name', null, ['class'=>'form-control']) !!}
                </div>
                <div class="form-group">
                    {!! Form::label('id', 'College:') !!}
                    {!! Form::select('college_id', [''=> 'Choose College'] + $colleges, null, ['class'=>'form-control'])
                    !!}
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                {!! Form::submit('Submit', ['class'=>'btn btn-primary', 'id'=>'submit']) !!}
                {!! Form::close() !!}
            </div>
        </div>
    </div>
</div>

            <!-- Delete Modal -->
                <div class="modal fade" id="promptModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                        aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">Delete Course</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    Are you sure you want to delete this course?
                                </div>
                                <div class="modal-footer">
                                    <form action="{{route('courses.destroy', 'delete')}}" method="POST">
                                        {{method_field('delete')}}
                                    {{csrf_field()}}
                                    <input type="hidden" name="course_val" id="course_id" value="">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                    {!! Form::submit('Delete', ['class'=>'btn btn-danger']) !!}
                                    {!! Form::close() !!}
                                    </form>
                                </div>
                            </div>
                        </div>
                    </div>    

<script>
    $(document).ready(function() {
        $('#promptModal').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget)
        var course_id = button.data('course') 
        var modal = $(this)
        modal.find('.modal-footer #course_id').val(course_id)
        })
    }); 
    
</script>
@stop

@extends('layouts.user_footer')